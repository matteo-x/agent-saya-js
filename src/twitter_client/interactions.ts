...
export class TwitterInteractionClient {
	...
	postClient: MixTwitterPostClient;

	constructor(
		client: ClientBase,
		post: MixTwitterPostClient,
		runtime: IAgentRuntime
	) {
		this.client = client;
		this.runtime = runtime;
		this.postClient = post;
	}
		...
		if (response.text) {
			try {
				const callback: HandlerCallback = async (response: Content) => {
					...
					if (this.postClient.shouldUseSdk()) {
						try {
							memories = await sendTweetV2(
								this.postClient,
							...
							console.log(
								`[TwitterInteractionClient] sendTweetV2 success, memory.length = ${memories.length}, tweet url = ${memories[0].content.url}, but [it may be send by spider]`
							);
						} catch (e) {
							console.log(
								`[TwitterInteractionClient] sendTweetV2 fail, will callback, respond tweetId = ${tweet.id}`,
								e
							);
