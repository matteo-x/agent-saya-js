...
/**
 * 1. not split tweet.
 * 2. send by official sdk with app key
 * @param postClient
 * @param content
 * @param roomId
 ...
 * @returns
 */
export async function sendTweetV2(
    ...
    console.log(
        `sendTweetV2 is called, roomId = ${roomId}, twitterUsername = ${twitterUsername}, inReplyTo = ${inReplyTo}, contentLength = ${content?.text?.length}`
    );
    const tweetChunks = [content.text];
    const sentTweets: Tweet[] = [];
    let previousTweetId = inReplyTo;

    for (const chunk of tweetChunks) {
        const result = await postClient.sendTweet(chunk, previousTweetId);
